            ...
            if (Object.keys(newMedia).includes(type)) {
                ...
                obj.save(function (err, updated) {
                    ...
                    if (type === 'lesson') {
                        ...
                        firebase.db.ref('message/lessons/' + updated._id + '/structure/').set(config.initOLesson, function(err) {
                            if (err) {
                                console.log(err)
                            } else {
                                firebase.db.ref('message/lessons/' + updated._id + '/structure/before/hook').update({show: data.prefs.hook})
                                firebase.db.ref('message/lessons/' + updated._id + '/structure/after/application').update({show: data.prefs.application})
                                firebase.db.ref('message/lessons/' + updated._id + '/structure/after/prayer').update({show: data.prefs.prayer})
                                // TODO: Increment user data
                                ...
                                if (data.template !== 'blank') {
                                    firebase.db.ref('message/lessons/' + updated._id + '/modules/').set()
                                } else {
                                    res.send(updated)
                                }
                            }
                        ...
                        // Add database reference to Firebase
                        firebase.db.ref('message/sermons/' + updated._id + '/structure/').set(config.initOSermon, function(err) {
                            if (err) {
                                console.log(err)
                            } else {
                                firebase.db.ref('message/sermons/' + updated._id + '/structure/before/hook').update({show: data.prefs.hook})
                                firebase.db.ref('message/sermons/' + updated._id + '/structure/after/application').update({show: data.prefs.application})
                                firebase.db.ref('message/sermons/' + updated._id + '/structure/after/prayer').update({show: data.prefs.prayer})
                                // TODO: Increment user data
                                // firebase.db.ref('users/' + decodedToken.uid + '/stats/num' + type.slice(1)).transaction(function (currentVal) {
                                //     return (currentVal || 0) + 1
                                // })
